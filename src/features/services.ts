...
/**
 * Finds the next service without reading the complete service archive. The
 * public viewer uses this for its first paint; the full picker follows later.
 */
export function getNextUpcomingService() {
  const sh = getSheetByName(SERVICES_SHEET);
  const lastRow = sh.getLastRow();
  const lastCol = sh.getLastColumn();
  ...
  const dates = dateIdx >= 0
    ? sh.getRange(2, dateIdx + 1, rowCount, 1).getValues()
    : [];
